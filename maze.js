...
class Maze {
  ...
  constructor(cols, rows, cellSize) {
    ...
    this.cols = cols;
    this.endColor = "#FF006E";
    this.mazeColor = "#00FF41";
    this.playerColor = "#00D9FF";
    this.rows = rows;
  ...
  redraw() {
    ...
    ctx.fillText("⭐", (this.cols - 0.5) * this.cellSize, (this.rows - 0.5) * this.cellSize);
    ...
    ctx.fillStyle = "#000";
    ctx.fillText("🔒", (player.col + 0.5) * this.cellSize, (player.row + 0.5) * this.cellSize);

  }

  checkWin() {
    return player.col === (this.cols - 1) && player.row === (this.rows - 1);
  }
...
function endGame(won) {
  ...
  if (won) {
    showPopup('✓ ESCAPED!', 'Congratulations! You escaped the maze!', true);
  } else {
